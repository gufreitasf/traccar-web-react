    ...
    render() {
        ...
        if (loading) {
            ...
            const updated = this.devicesBetweenTime( devices,
                                                    new Date(loadTime.getTime() - 6*3600000), //Última 6 horas 
                                                    loadTime );
            const outdated = this.devicesBetweenTime( devices,
                                                    new Date(loadTime.getTime() - 72*3600000), //Entre 6 e 72h
                                                    new Date(loadTime.getTime() - 6*3600000) );
            const maintenance = this.devicesBetweenTime( devices,
                                                    new Date(0), //Maior que 72h
                                                    new Date(loadTime.getTime() - 72*3600000) );
            return (
